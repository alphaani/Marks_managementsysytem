...
import {
  BookOpen, FileText, Settings, LogOut, GraduationCap, Menu, X
} from 'lucide-react';
...
const menuItems = [
  { id: 'overview', label: 'My Dashboard', icon: BookOpen, path: '/student' },
  { id: 'marks', label: 'View Marks', icon: FileText, path: '/student/marks' },
  { id: 'corrections', label: 'Correction Requests', icon: Settings, path: '/student/corrections' }
];
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        ...
        <div className="flex items-center justify-between h-16 px-6 border-b shrink-0">
          <div className="flex items-center">
            ...
            <span className="ml-2 text-xl font-semibold text-gray-800">Student Portal</span>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50">
          <div className="max-w-7xl mx-auto">
            <Routes>
              ...
              <Route path="corrections" element={<StudentCorrections />} />
            </Routes>
          </div>
        </main>
      </div>
